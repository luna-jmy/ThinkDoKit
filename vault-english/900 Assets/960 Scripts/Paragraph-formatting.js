module.exports = async (params) => {
    ...
    if (lines.length > 0 && lines[0].trim() === '---') {
        // Look for the end of the YAML header
    ...
    const paragraphFormatting = (currentContent) => {
        // 1. Preserve original trailing whitespace and process the main body
        const contentToProcess = currentContent.trimEnd();
        const trailingWhitespace = currentContent.substring(contentToProcess.length);

        // 2. Start with a "condensed" version of the text (no empty lines)
        ...
        const result = [];
        let inCodeBlock = false;
        result.push(initialLines[0]); // Add the first line to the result
        ...
        // 3. Iterate through the rest of the lines and add blank lines based on context
        ...
        // 4. Reconstruct the body and handle the end of the file
    ...
    // Reconstruct the full content with YAML header (if exists)
    let newContent;
    if (hasYamlHeader) {
